...
def Config_Sensor_Archivo(file_path):
    global correcto, fuera_rango, error
...
def Sensor_Humo(sensor_id):
    ...
    while True:
        ...
        time.sleep(3)  # Cada 3 segundos según el documento
...
def Sensor_Temperatura(sensor_id):
    ...
    socket = context.socket(zmq.PUSH)
    socket.connect("tcp://localhost:5555")
    global cont_total, cont_correcto, cont_fuera_rango, cont_error

    while True:
        temperatura = 0
        # Cambiar de acuerdo a los porcentajes
        if cont_total == 0:
